...
class UserDataHelper {
  ...
  /// Refresh user data for a specific user id and update local storage.
  /// This does not rely on the cached `current_user` and is useful when
  /// we want to force a fresh read from the backend after making changes
  /// (for example after uploading a profile image or updating the profile).
  Future<bool> refreshUserDataFromDatabaseById(String userId) async {
    try {
      if (userId.isEmpty) {
        debugPrint('Empty userId provided for refresh', wrapWidth: 1024);
        return false;
      }

      final getUserUseCase = di.sl<GetUserByIdUseCase>();
      final result = await getUserUseCase(GetUserByIdParams(userId: userId));
      ...
      return result.fold(
        ...
        (freshUser) async {
          if (freshUser != null) {
            await saveCurrentUser(freshUser);
            debugPrint(
              'User data refreshed successfully by id',
              wrapWidth: 1024,
            );
            return true;
          }
      ...
      debugPrint('Error refreshing user data by id: $e', wrapWidth: 1024);
      return false;
    }
  }
  ...
  /// Force complete refresh of user data and clear cache first
  /// This ensures we get the latest data from database
  Future<bool> forceCompleteRefresh() async {
    try {
      final currentUser = getCurrentUser();
      if (currentUser?.id == null) {
        debugPrint('No current user found for force refresh', wrapWidth: 1024);
        return false;
      }
      ...
      final success = await refreshUserDataFromDatabaseById(currentUser!.id!);

      if (success) {
        debugPrint('Force refresh completed successfully', wrapWidth: 1024);
      } else {
        // If refresh fails, restore the original user
        await saveCurrentUser(currentUser);
        debugPrint(
          'Force refresh failed, restored original user',
          wrapWidth: 1024,
        );
      }
      ...
      debugPrint('Error in force refresh: $e', wrapWidth: 1024);
      return false;
    }
  }
}
