...
class _NotificationSettingsWidgetState
  ...
  final _notificationService = NotificationPreferencesService();
  ...
  Future<void> _loadNotificationSettings() async {
    try {
      final reportsNotifications =
          await _notificationService.isReportsNotificationsEnabled();
      final newsNotifications =
          await _notificationService.isNewsNotificationsEnabled();
      final securityNotifications =
          await _notificationService.isSecurityNotificationsEnabled();

      if (mounted) {
        setState(() {
          _reportsNotifications = reportsNotifications;
  ...
  Future<void> _updateNotificationsSetting(bool value) async {
    if (!value) {
      final shouldDisable = await _showDisableConfirmationDialog();
      if (!shouldDisable) return;
    }

    context.read<SettingsBloc>().add(SettingsNotificationChanged(value));
    ...
    return BlocBuilder<SettingsBloc, SettingsState>(
      buildWhen: (previous, current) {
        ...
        if (previous is SettingsLoaded && current is SettingsLoaded) {
          return previous.settings.notificationsEnabled !=
                  current.settings.notificationsEnabled ||
              previous.settings.soundEnabled != current.settings.soundEnabled ||
              previous.settings.vibrationEnabled !=
                  current.settings.vibrationEnabled;
        }
        return true;
      },
      builder: (context, state) {
        final notificationsEnabled =
            state is SettingsLoaded
                ...
                : true;
        final soundEnabled =
            state is SettingsLoaded ? state.settings.soundEnabled : true;
        final vibrationEnabled =
            state is SettingsLoaded ? state.settings.vibrationEnabled : true;
          ...
          children: [
            ...
            if (notificationsEnabled) ...[
              ...
              _buildNotificationTile(
                ...
                title: 'الصوت',
                subtitle: 'تشغيل الصوت مع الإشعارات',
                value: soundEnabled,
                onChanged: (value) async {
                  SettingsService().updateSound(context, value);
              ...
              _buildNotificationTile(
                ...
                onChanged: (value) async {
                  SettingsService().updateVibration(context, value);
              ...
              _buildNotificationTile(
                ...
                subtitle: 'تحديثات حول التقارير المقدمة',
                value: _reportsNotifications,
                onChanged: (value) async {
                  setState(() {
                    _reportsNotifications = value;
                  });
                  await _notificationService.setReportsNotificationsEnabled(
                    value,
                  );
                },
              ),
              ...
              _buildNotificationTile(
                ...
                onChanged: (value) async {
                  ...
                  await _notificationService.setNewsNotificationsEnabled(value);
                },
              ),
              ...
              _buildNotificationTile(
                ...
                subtitle: 'تنبيهات هامة حول الأمان',
                value: _securityNotifications,
                onChanged: (value) async {
                  setState(() {
                    _securityNotifications = value;
                  });
                  await _notificationService.setSecurityNotificationsEnabled(
                    value,
                  );
                },
                isImportant: true,
  ...
  Widget _buildNotificationTile({
    ...
    final switchKey = ValueKey('${title.replaceAll(' ', '_')}_switch');
