...
class _HeatMapWidgetState extends State<HeatMapWidget>
            ...
            FlutterMap(
              ...
              children: [
                ...
                if (state is HeatmapLoaded)
                  ..._buildEnhancedHeatmapLayers(state.reports),
  ...
  List<Widget> _buildEnhancedHeatmapLayers(List<dynamic> reports) {
    List<Widget> layers = [];
    ...
    for (final report in reports) {
      ...
      governorateReports.putIfAbsent(governorate, () => []).add(report);
    ...
    _addGovernorateMarkers(governorateMarkers, governorateReports);
    ...
    for (final entry in areaReports.entries) {
      ...
      final crimeTypeStats = _analyzeCrimeTypes(reportsInArea);
      ...
      final baseRadius = _calculateRadius(intensity);
      ...
      for (int i = 4; i >= 1; i--) {
        ...
        heatCircles.add(
          CircleMarker(
            point: location,
            radius: baseRadius * (radiusMultiplier + 0.3),
            color: mainColor.withOpacity(opacityLevel),
            borderColor: mainColor.withOpacity(opacityLevel * 2),
      ...
      reportMarkers.add(
        Marker(
          ...
          child: GestureDetector(
            onTap:
                () => _showEnhancedAreaDetails(reportsInArea, crimeTypeStats),
            child: Stack(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Icon(
                        _getCrimeTypeIcon(dominantCrimeType),
                        color: Colors.white,
                        size: 18.sp,
                      ),
                      Text(
      ...
      for (int i = 0; i < reportsInArea.length && i < 8; i++) {
        final angle = (i * 45.0) * (3.14159 / 180); // تحويل إلى راديان
        final offsetLat = location.latitude + (0.001 * math.cos(angle));
        final offsetLng = location.longitude + (0.001 * math.sin(angle));
        ...
        reportMarkers.add(
          Marker(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: _getCrimeTypeColor(reportsInArea[i].reportType),
                shape: BoxShape.circle,
    ...
    if (governorateMarkers.isNotEmpty) {
      layers.add(MarkerLayer(markers: governorateMarkers));
    }
  ...
  void _addGovernorateMarkers(
    ...
    final Map<String, LatLng> governorateLocations = {
      ...
      'البحر الأحمر': const LatLng(26.1062, 33.8116),
      'الوادي الجديد': const LatLng(25.4500, 30.5500),
      'مطروح': const LatLng(31.3543, 27.2373),
      'شمال سيناء': const LatLng(31.1313, 33.8116),
      'جنوب سيناء': const LatLng(28.9717, 33.6175),
    };

    for (final entry in governorateReports.entries) {
      ...
      final crimeLevel = _getCrimeLevel(reportCount);
      ...
      governorateMarkers.add(
        Marker(
          ...
          height: 80.h,
          child: GestureDetector(
            onTap: () => _showGovernorateDetails(governorate, reports),
            child: Column(
              ...
              children: [
                    ...
                    builder: (context, value, child) {
                      return Transform.translate(
                        ...
                        child: Icon(
                          ...
                          color: Colors.red.withOpacity(0.7 + 0.3 * value),
                          size: 24.sp,
                        ),
                      );
                    },
                ...
                Container(
                  ...
                  height: 50.h,
                  decoration: BoxDecoration(
                    color: _getCrimeLevelColor(crimeLevel),
                    shape: BoxShape.circle,
                    border: Border.all(color: Colors.white, width: 3),
                    boxShadow: [
                      BoxShadow(
                        color: _getCrimeLevelColor(crimeLevel).withOpacity(0.4),
                        blurRadius: 12,
                        spreadRadius: 4,
                      ),
                    ],
  ...
  void _showGovernorateDetails(String governorate, List<dynamic> reports) {
    final crimeStats = _analyzeCrimeTypes(reports);
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.report, color: Colors.blue, size: 32.sp),
                      SizedBox(width: 12.w),
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                ...
                Expanded(
                  child: ListView.builder(
                    itemCount: crimeStats.entries.length,
                    itemBuilder: (context, index) {
                      final entry = crimeStats.entries.elementAt(index);
                      final percentage =
                          (entry.value / reports.length * 100).round();
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Icon(
                              _getCrimeTypeIcon(entry.key),
                              color: _getCrimeTypeColor(entry.key),
                              size: 24.sp,
                            ),
                            SizedBox(width: 12.w),
                            Expanded(
    ...
    if (state is HeatmapLoaded) {
      ...
      final nearbyReports =
          state.reports.where((report) {
            ...
            return distance <= 5000; // ضمن دائرة نصف قطرها 5 كم
          }).toList();
  ...
  void _showEnhancedAreaDetails(
    ...
    final location = reports.first.governorate ?? 'غير محدد';
          ...
          (context) => Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Container(
                      padding: EdgeInsets.all(12.w),
                      decoration: BoxDecoration(
                        color: _getCrimeColor(totalReports).withOpacity(0.1),
                        borderRadius: BorderRadius.circular(12.r),
                      ),
                      child: Icon(
                        Icons.location_on,
                        color: _getCrimeColor(totalReports),
                        size: 24.sp,
                      ),
                    ),
                    SizedBox(width: 16.w),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: [
                        _getCrimeColor(totalReports).withOpacity(0.1),
                        _getCrimeColor(totalReports).withOpacity(0.05),
                      ],
                    ),
                    borderRadius: BorderRadius.circular(12.r),
                    border: Border.all(
                      color: _getCrimeColor(totalReports).withOpacity(0.3),
                    ),
                  ),
                  child: Row(
                    children: [
                ...
                Expanded(
                  child: ListView.builder(
                    itemCount: crimeTypeStats.length,
                    itemBuilder: (context, index) {
                      final entry = crimeTypeStats.entries.elementAt(index);
                      final crimeType = entry.key;
                      final count = entry.value;
                      final percentage = (count / totalReports * 100).round();
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: FractionallySizedBox(
                                widthFactor: count / totalReports,
                                child: Container(
                                  decoration: BoxDecoration(
                                    color: _getCrimeTypeColor(crimeType),
                                    borderRadius: BorderRadius.circular(3.r),
