...
class _SignupPageState extends State<SignupPage> {
    ...
    if (state is SignupError ||
      ...
      WidgetsBinding.instance.addPostFrameCallback((
        ...
        try {
          ...
          print(
            '❌ Error during post-signup cleanup: $e',
          );
    ...
    switch (_currentStep) {
      ...
      case 4: // Location step (now step 4, not 5)
        final canProceed =
            ...
            _selectedCity != null;
        print(
          '🔍 Step 4 - Can proceed: $canProceed (Gov: $_selectedGovernorate, City: $_selectedCity)',
        ); // Debug log
        return canProceed;
  ...
  bool _isDataValid() {
    try {
      ...
      print(
        '🔍 Validating data for step $_currentStep',
      );
      ...
      if (_selectedUserType == UserType.citizen) {
        ...
        if (nationalId.length != 14 ||
          ...
          print(
            '❌ Invalid national ID: $nationalId',
          );
          return false;
        }
      } else {
  ...
  void _nextStep() async {
    try {
      ...
      print(
        '🚀 _nextStep called for step $_currentStep',
      );
      print(
        '📊 Can proceed: ${_canProceedToNext()}',
      );
      ...
      if (_currentStep == 0) {
        ...
        if (_isDataValid()) {
          print(
            '✅ Data is valid, proceeding to next step',
          );
          _proceedToNextStep();
        } else {
          print(
            '❌ Data validation failed, staying on current step',
          );
          final errorMessage =
              _getValidationError();
        ...
        if (_selectedGovernorate != null &&
            _selectedCity != null) {
          print(
            '✅ Location selected, proceeding to review',
          );
          _proceedToNextStep();
        } else {
          _showValidationError(
  ...
  Future<void> _verifyOTP() async {
    ...
    if (_otpCode.trim().isEmpty ||
        _otpCode.contains(RegExp(r'[^0-9]'))) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  void _debugCurrentState() {
    ...
    print(
      '👤 Selected User Type: $_selectedUserType',
    );
    print(
    ...
    if (_currentStep == 3) {
      ...
      print(
        '📋 Required fields for step 3: $requiredFields',
      );
      for (String field in requiredFields) {
        final value = _userData[field];
        print(
          '   $field: "${value ?? ''}" (${value?.isEmpty == true ? 'EMPTY' : 'OK'})',
        );
      }
    }
    print('================================');
